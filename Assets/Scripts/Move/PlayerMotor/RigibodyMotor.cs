...
namespace FireControl.Motor
{
    ...
    public class RigibodyMotor : MonoBehaviour
    {
        ...
        public void Move(float horizontal, float vertical)
        {
            ...
            desiredVelocity = desiredVelocity * playerInfo.runSpeed;
        ...
        void AdjustVelocity()
        {
            ...
            //确定实际上在这个平面上的X移动值
            float currentX = Vector3.Dot(velocity, xAixs);
            //确定实际上在这个平面上的Z移动值
        ...
        bool SnapToGround()
        {
            ...
            RaycastHit hit;
            if (!Physics.Raycast(rigidbody.position, -Vector3.up, out hit, probeDistance, probeMask))
                return false;
        ...
        Common.Handler.HandlerList.INonReturnAndNonParam onWaitEnd;
        ...
        void Rotate()
        {
            ...
            //渐变模式
            angle.y = Mathf.MoveTowardsAngle(angle.y, targetRotateY, playerInfo.rotateSpeed);
