...
namespace CustomRP.GPUPipeline
{
    public struct WaterGroupData
    {
        ...
        public Vector4 buttomPlane; //底部平面
    ...
    public class NoiseWater : DrawWaterBase
    {
        ...
        private Vector4 preButtomPlane;
        ...
        public GameObject RunWater()
        {
            ...
            if (groups[circulatePos].lifeTime.y > Time.time || Time.time - preTime < 0.05f)
                return null;
            preTime = Time.time;
            ...
            if (prePosition.y != transform.position.y)
            {
                ...
                if (Physics.Raycast(transform.position, Vector3.down, out raycastHit, float.MaxValue, rayLayer))
                {
                    ...
                    preButtomPlane = GetPlane(raycastHit.normal, raycastHit.point);
                }
                else
                    preButtomPlane = GetPlane(Vector3.up, 
                        new Vector3(transform.position.x, buttonY, transform.position.z));  //默认底面
            }
            ...
            if (transform.forward.y >= 0)
            {
                ...
                {
                    SetRayPoint(raycastHit, rayDis * transform.forward, 
                        upTime, circulatePos, preButtomPlane);
                    circulatePos++;
            ...
            {
                SetRayPoint(raycastHit, rayDis * transform.forward, 
                    downTime + upTime, circulatePos, preButtomPlane);
                circulatePos++;
            ...
            SetRayPoint(raycastHit, rayDis * transform.forward, upTime, circulatePos, preButtomPlane);
            circulatePos++;
            //Debug.Log("Two Point");
        ...
        /// <summary>      /// 设置碰撞数据        /// </summary>
        /// <param name="raycastHit">碰撞数据</param>
        /// <param name="beginSpeed">初始出射速度</param>
        /// <param name="moveTime">需要移动的时间</param>
        /// <param name="index">要设置的编号</param>
        /// <param name="buttomPlane">底平面，对碰撞到墙面时水能够流到地上</param>
        private void SetRayPoint(RaycastHit raycastHit, Vector3 beginSpeed,
            float moveTime, int index, Vector4 buttomPlane)
        {
            ...
            groups[index].buttomPlane = buttomPlane;
